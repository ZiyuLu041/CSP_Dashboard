#!/usr/bin/env python3
"""
CSP Multi-Ticker WebSocket Output Pipeline with Real Polygon Data
...
@csp.graph
def crypto_multi_ticker_websocket_pipeline(port: int):
    """CSP pipeline using dynamic processing for multiple tickers"""
    ...
    # Create tables with delta_updates=True for optimization
    # trades: use timestamp index for efficient updates (delta updates on timestamp changes)
    # statistics: use pair index - only latest stats per pair, with delta updates
    ...
    # Print the collected statistics (as a dictionary)
    # csp.print("collected_stats", collected_stats)
    stats_list = dict_to_list(collected_stats)
    # csp.print("stats_list", stats_list)
    # Use csp.unroll to convert list to individual ticks
    individual_stats = csp.unroll(stats_list)
    individual_stats = csp.filter(individual_stats.volumn_60s > 1000, individual_stats)
    # csp.print("individual_stats", individual_stats)
    # Publish to WebSocket table
    stats_table.publish(individual_stats)

def main():
    port = 7678  # Use different port from single-ticker version
    print(f"Starting CSP Multi-Ticker WebSocket pipeline")
    print(f"All crypto pairs → CSP dynamic processing → Native WebSocket")
